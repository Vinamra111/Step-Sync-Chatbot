  ...
  Future<void> initialize({bool loadPreviousConversation = false}) async {
    try {
      ...
      await _sendBotMessage(
        ChatMessage.bot(
          text: 'Hi! I\'m Step Sync Assistant. Let me check your setup...',
        ),
      );

      // Check tracking status proactively
      await _checkAndReportTrackingStatus();
    } catch (e) {
      state = state.copyWith(
    ...
    try {
      ...
      return {
        ...
        'primaryIssue': diagnosticResult.issues.isNotEmpty
            ...
            : 'No issues detected',
        'platformAvailable': diagnosticResult.isPlatformAvailable,
      };
    } catch (e) {
